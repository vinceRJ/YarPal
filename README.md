# 🌤️ LarPal : Votre copilote d'investissement intelligent

Bienvenue dans l'univers de **LarPal**, un assistant financier personnel conçu pour simplifier la gestion de vos portefeuilles **PEA** et **CTO**. Né d'un besoin de clarté et d'analyse en temps réel, LarPal utilise la puissance des agents autonomes (GPT-4o et Gemini 1.5 Pro) pour transformer vos données boursières en décisions stratégiques.

## 🌟 Pourquoi utiliser LarPal ?
- **Tout au même endroit** : Visualisez votre PEA et votre CTO dans une interface unique, chaleureuse et fluide.
- **L'intelligence à votre service** : LarPal ne se contente pas d'afficher des chiffres. Il explique les hauts et les bas du marché grâce à ses agents IA.
...
```bash
git clone https://github.com/vinceRJ/LarPal.git
cd LarPal
pip install -r requirements.txt
```

### 2. Configurez vos "cerveaux" (Clés API)
Pour que LarPal puisse réfléchir et chercher des infos, il a besoin de ses clés API. Chaque clé apporte un "super-pouvoir" spécifique :
...
*   **Tavily Key (Standard)** : Ce sont les **Yeux de LarPal**. Elle lui permet de fouiller le Web pour trouver les news qui font bouger vos actions.
...
2. Copiez et remplissez les lignes suivantes :
   ...
   LARPAL_PASSWORD=votre_code_secret
...
Chaque utilisateur peut déployer **sa propre instance privée** de LarPal gratuitement.
...
4. Dans **Settings → Secrets**, ajoutez vos clés au format TOML :
   ...
   GEMINI_API_KEY = "AIza..."
   TAVILY_API_KEY = "tvly-..."
   LARPAL_PASSWORD = "votre_mot_de_passe_fort"
   USER_NAME = "VotrePrenom"
   ```
5. Cliquez sur **Deploy** — votre LarPal est en ligne en moins de 2 minutes.
...
- **Solution simple** : utilisez LarPal en local (`streamlit run src/app.py`) et conservez votre `portfolio.json` sur votre machine.
...
Votre vie privée est sacrée. LarPal est conçu pour que vos données de portefeuille (`portfolio.json`) et vos clés API (`.env`) restent **exclusivement sur votre machine**. Elles ne sont jamais envoyées sur GitHub.

### Protection contre les injections de prompt

LarPal intègre plusieurs défenses contre les tentatives de manipulation des agents IA :
...
> Ces défenses réduisent significativement le risque, mais aucun système n'est à 100 % inviolable. Ne laissez pas LarPal accéder à des données sensibles non liées à vos finances.
